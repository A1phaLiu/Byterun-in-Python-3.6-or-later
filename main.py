...
class VirtualMachine(object):
    ...
    def parse_byte_and_args(self):
        ...
        if byteCode >= dis.HAVE_ARGUMENT:  # 通过指令字节码判断指令是否有参数  < HAVE_ARGUMENT：无  >= HAVE_ARGUMENT：有
            # 解析指令参数
            arg = f.code_obj.co_code[f.last_instruction : f.last_instruction + 1]  # 指令中后面代表参数的字节码
            f.last_instruction += 1  # 后移读取指令的指针
    ...
    def dispatch(self, byte_name, argument):
        ...
        try:
            bytecode_fn = getattr(self, "byte_%s" % byte_name, None)  # 查找并取得指令名称对应的指令方法
    ...
    def push_block(self, b_type, handler=None):  # 压入块栈，接受块类型和处理器参数
        level = len(self.frame.stack)  # 当前数据栈的层数
        self.frame.block_stack.append(Block(b_type, handler, level))  # 新增一个块对象Block（数据类型是namedtuple）
    ...
    def byte_LOAD_FAST(self, name):  # 压入局部变量到数据栈顶部
    ...
    def byte_BUILD_MAP(self, size):  # 构建空字典，忽略size参数
        self.push({})
    ...
    def byte_CALL_FUNCTION(self, arg):  # 调用函数指令对应方法
        lenKw, lenPos = divmod(arg, 256)  # 计算关键字参数和位置参数数量 KWargs not supported here
